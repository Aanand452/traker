...
import "./style.css";
import Calendar from "../SubCalendar";
...
class SideNavBar extends Component {
    ...
    //         console.log(this.state.subevents.filter(reg => reg.isChecked).map(subevent => subevent.title));
    //     });
    // }
    handleSubEventsCheckbox = (index) => {
        const subevents = this.state.subevents;
                    ...
                    {this.state.events.map((item, index) => (
                        <div className={`slds-col_padded ${item.title.replace(/ /g, "")}_checkbox`}>
                            ...
                            <Checkbox className="mycheckbox"
                                style={{
                                    borderRadius: "25px", fontWeight: "bold", fontSize: "0.75rem", cursor: 'pointer', color: 'blue'
                                }}
                                indeterminate
                                assistiveText={{
                                    label: item.title,
