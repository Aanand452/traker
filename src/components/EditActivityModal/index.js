...
// ACTIONS
import { editItem } from "../../actions/DataTable";
...
class EditActivityModalComponent extends Component {
  ...
  getActicityById = async (id) => {
    ...
    try {
      var activityProgram = this.state.program.filter(
        (el) => el.program_id === result.programId
      );
      var activityRegion = this.state.region.filter(
        (el) => el.region_id === result.regionId
      );
      var activityFormat = this.state.format.filter(
        (el) => el.format_id === result.formatId
      );

      const assets = result.asset
        ? result.asset.split(", ").map((asset) => ({
            id: asset,
            title: asset,
            label: asset,
          }))
  ...
  handleChange = (e) => {
    ...
    if (e.target.id === "asset") {
      errors = {
        ...errors,
        asset: !this.isUrl(e.target.value),
        assetRepeat: false,
      };
  ...
  editTable = async (e) => {
    ...
    try {
      ...
      if (this.state.asset) {
        if (
          this.state.assets.some(
            (val) => val.title.toLowerCase() === this.state.asset.toLowerCase()
          )
        ) {
      ...
      let body = {
        ...
        formatId:
          this.state.formatSelection[0] && this.state.formatSelection[0].id,
        abstract: this.state.abstract,
        regionId:
          this.state.regionSelection[0] && this.state.regionSelection[0].id,
        ...
        programId:
          this.state.programSelection[0] && this.state.programSelection[0].id,
      };
      let token = getCookie("token").replaceAll('"', "");
      ...
      let response = await fetch(
        `${this.API_URL}/activity/${this.props.data.activityId}`,
        config
      );
      ...
      if (response.status === 200) {
        this.props.editItem(this.props.dataTable.items, {
          campaignId: this.state.campaignId,
          program:
            this.state.programSelection[0] &&
            this.state.programSelection[0].program_id,
          format:
            this.state.formatSelection[0] &&
            this.state.formatSelection[0].format_id,
          region:
            this.state.regionSelection[0] &&
            this.state.regionSelection[0].region_id,
          title: this.state.title,
          abstract: this.state.abstract,
          startDate: this.state.startDate,
          endDate: this.state.endDate,
          ...
          <section className="slds-p-around_large">
            ...
            <div className="slds-form-element slds-m-bottom_large">
              <Textarea
                ...
                label="Abstract"
                errorText={
                  this.state.errors.abstract ? "This field is required" : ""
                }
            ...
            <div style={{ display: "table", width: "100%" }}>
              <div style={{ float: "left", width: "25%" }}>
                <div className="slds-form-element slds-m-bottom_large">
                  <Combobox
                    ...
                    variant="readonly"
                    errorText={
                      this.state.errors.formatId ? "This field is required" : ""
                    }
                  />
                </div>
              </div>
              <div style={{ float: "left", width: "25%" }}>
                <div className="slds-form-element slds-m-bottom_large">
                  <Combobox
                    ...
                    variant="readonly"
                    errorText={
                      this.state.errors.regionId ? "This field is required" : ""
                    }
                  />
                </div>
              </div>
              <div style={{ float: "left", width: "25%" }}>
                <div
                  className={`slds-m-bottom_large slds-col slds-size_1-of-1 ${
