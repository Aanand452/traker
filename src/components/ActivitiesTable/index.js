...
const DropDownCellAsset = ({ children, ...props }) => {
  ...
  return (
    <CustomDataTableCell {...props}>{props.item[items]}</CustomDataTableCell>
  );
};
...
class Table extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.data !== prevProps.data) {
      ...
      if (!this.state.defaultUserFilter)
        if (
    ...
    this.setState({
      columnWidth: {
        Owner: localStorage.getItem("Owner")
          ? Number(localStorage.getItem("Owner"))
          : (document.body.clientWidth - 52) / 10,
        Program: localStorage.getItem("Program")
          ? Number(localStorage.getItem("Program"))
          : (document.body.clientWidth - 52) / 10,
        "Campaign ID": localStorage.getItem("Campaign ID")
          ? Number(localStorage.getItem("Campaign ID"))
          : (document.body.clientWidth - 52) / 10,
        Title: localStorage.getItem("Title")
          ? Number(localStorage.getItem("Title"))
          : (document.body.clientWidth - 52) / 10,
        Format: localStorage.getItem("Format")
          ? Number(localStorage.getItem("Format"))
          : (document.body.clientWidth - 52) / 10,
        Abstract: localStorage.getItem("Abstract")
          ? Number(localStorage.getItem("Abstract"))
          : (document.body.clientWidth - 52) / 10,
        Region: localStorage.getItem("Region")
          ? Number(localStorage.getItem("Region"))
          : (document.body.clientWidth - 52) / 10,
        "Start date": localStorage.getItem("Start date")
          ? Number(localStorage.getItem("Start date"))
          : (document.body.clientWidth - 52) / 10,
        "End date": localStorage.getItem("End date")
          ? Number(localStorage.getItem("End date"))
          : (document.body.clientWidth - 52) / 10,
        Assets: localStorage.getItem("Assets")
          ? Number(localStorage.getItem("Assets"))
          : (document.body.clientWidth - 52) / 10,
      },
    });
  ...
  async checkFormat() {
    try {
      ...
      if (response.status === 200) {
        ...
        var categoryrowlist=[]
        formats.map((item)=>{
          if(categoryrowlist.indexOf(item.category)===-1 && item.category !==null ){
  ...
  getDefaultFornats = (formats) => {
    ...
    var categorylist=[]
    formats.map((item)=>{
      if(categorylist.indexOf(item.category)===-1 && item.category !==null ){
  ...
  closeHistoricModal = () =>
    this.setState({ historicModalIsOpen: false, historicDate: {} });

  closeDetailModal = () => this.setState({ detailModalIsOpen: false });

  handleHistoricDate = (name, value) =>
    this.setState({
      historicDate: { ...this.state.historicDate, [name]: value },
    });
  ...
  exportPdf = () => {
    const exportHeaders = [
      ["Program", "Title", "Format", "Region", "Start Data"],
    ];
  ...
  actions = () => (
    <PageHeaderControl>
      <ButtonGroup id="button-group-page-header-actions-history">
        <Tooltip
          content={
            this.state.isCalanderView ? "Open Calendar View" : "Open List View"
          }
        >
          <Button onClick={this.calendarViewBtn}>
            <Icon
        ...
        <Button
          ...
          onClick={() =>
            this.setState({
              historicModalIsOpen: !this.state.historicModalIsOpen,
            })
          }
  ...
  filter = (arr, functionFilters) => {
    return arr.filter((row) => {
      for (const property in functionFilters) {
        if (
          !functionFilters[property](
            row[property] && row[property].toLowerCase()
          )
        )
          return false;
      }
    ...
    if (region) {
      try {
        if (region.length > 1 || region[0].id !== "all") {
          const filteredPrograms = this.state.programs.filter((x) =>
            this.state.regionsSelected.find((y) => y.id === x.target_region)
          );
          this.setState({
  ...
  updateFilters = () => {
    ...
    this.setState({
      regionsSelected: slectedRegioins,
      programSelected,
  ...
  getFilteredData = () => {
    ...
    const filteredData = arr.filter((row) => {
      if (
        ...
        !slectedRegioins.includes(row.regionId)
      )
        return false;
      if (
        ...
        return false;
      if (
        formatsSelected.length > 0 &&
        !formatsSelected.includes("All") &&
        !formatsSelected.includes(row.formatId)
      )
        return false;
      return true;
    });
  ...
  getUserDefaultFilter = async () => {
    try {
      ...
      const request = await fetch(
        `${this.API_URL}/user-filter/` + this.state.userId,
        config
      );
  ...
  saveAsDefaultFilter = async (e) => {
    try {
      ...
      let response =
        this.state.defaultUserFilter.length !== 0
          ? await fetch(
              `${this.API_URL}/user-filters/${this.state.defaultUserFilter.user_filter_id}`,
              config
            )
          : await fetch(`${this.API_URL}/user-filters`, config);
    ...
    if (name === "regionId" && value !== "") {
      if (value[0].id !== "all") {
        this.setState({
          filteredPrograms: [
            { label: "All", id: "all" },
            ...this.state.programs.filter(function (program) {
  ...
  onChangeDate = (dates) => {
    ...
    this.setState({
      ...
      calendarView: {
        date: start,
        view: this.state.calendarView.view,
        action:
          moment(start, "DD/MM/YYYY") > moment(new Date(), "DD/MM/YYYY")
            ? "NEXT"
            : "PREV",
      },
    });
  ...
  onClickToday = () => {
    this.setState({
      ...
      calendarView: {
        date: new Date(),
        view: this.state.calendarView.view,
        action: this.state.calendarView.action,
      },
    });
  };
  ...
  handleFilterChange = (event) => {
    ...
    if (event.target.value.length > 0) {
      ...
      if (
        this.state.formatsSelected.length > 0 ||
        this.state.programSelected.length > 0 ||
        this.state.regionsSelected.length > 0
      ) {
        this.getFilteredData();
        ...
        this.handlePagination(
          this.state.data.slice(0, this.state.pageLimit),
  ...
  formatsWithIcon = (formats) => {
    return formats.map((elem) => ({
      ...elem,
      ...{
        icon: <Icon assistiveText={{ label: "Account" }} category="standard" />,
      },
    }));
  };
  ...
  setCalendarView = () => {
    ...
    if (view === "month") {
      this.setState({
        calendarView: { date: date, view: "week", action: action },
      });
    } else {
      this.setState({
        calendarView: { date: date, view: "month", action: action },
      });
    }
  };
    ...
    return (
      <Container id="outer-container">
        ...
        {this.state.detailModalIsOpen && (
          <ViewActivityModal closeDetailModal={this.closeDetailModal} />
        )}
        ...
        {
          <Modal
            isOpen={this.state.openNew}
            onRequestClose={() => this.setState({ openNew: false })}
            size="medium"
          >
        ...
        <Card
                      ...
                      <div style={{ float: "left" }}>
                        <Button
                          ...
                          style={{
                            ...
                            paddingRight: "10px",
                            color:
                              this.state.calendarView.view === "month"
                                ? "white"
                                : "black",
                            background:
                              this.state.calendarView.view === "month"
                      ...
                      <div style={{ float: "left" }}>
                        ...
                        <Button
                          ...
                          style={{
                            ...
                            paddingRight: "10px",
                            color:
                              this.state.calendarView.view === "week"
                                ? "white"
                                : "black",
                            background:
                              this.state.calendarView.view === "week"
          ...
          icon={
            <div style={{ display: "table" }}>
              <div style={{ float: "left" }}>
                <Button onClick={this.openMenu} variant="icon">
                  <Icon
        ...
        >
          ...
          >
                  ...
                  events={{
                    ...
                    onRequestClose: () => {
                      ...
                      if (this.state.regionsSelected.length > 1) {
                        this.setState({
                          regionsSelected: this.state.regionsSelected.filter(
                            (x) => {
                              return x.id !== "all";
                            }
                          ),
                      ...
                      if (this.state.regionsSelected.length === 0) {
                        this.setState({
                          regionsSelected: this.state.regions.filter((x) => {
                            return x.id === "all";
                          }),
                        });
                      }
                  ...
                  events={{
                    ...
                    onRequestClose: () => {
                      ...
                      if (this.state.programSelected.length > 1) {
                        this.setState({
                          programSelected: this.state.programSelected.filter(
                            (x) => {
                              return x.id !== "all";
                            }
                          ),
                      ...
                      if (this.state.programSelected.length === 0) {
                        this.setState({
                          programSelected: this.state.programs.filter((x) => {
                            return x.id === "all";
                          }),
                        });
                      }
            ...
            {this.state.openMenuBar && (
              <MultiSelectContainer style={{ paddingBottom: "25px" }}>
                <Combobox
                  multiple
                  isOpen={this.state.isFormatFilterOpen}
                  labels={{
                  ...
                  events={{
                    ...
                    onRequestClose: () => {
                      ...
                      if (this.state.formatsSelected.length > 1) {
                        this.setState({
                          formatsSelected: this.state.formatsSelected.filter(
                            (x) => {
                              return x.id !== "all";
                            }
                          ),
                      ...
                      if (this.state.formatsSelected.length === 0) {
                        this.setState({
                          formatsSelected: this.state.formats.filter((x) => {
                            return x.id === "all";
                          }),
                        });
                      }
            ...
            {!this.state.isCalanderView && (
              ...
              <DataTable
                ...
                className={`${
                  ...
                  this.state.displayedData.length < 5
                    ? "padding_bottom"
                    : ""
                }`}
              >
